...
void ST7565_fillrect(uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint8_t color) {
  ...
  // just pixels - but fast with internal buffer!
  for (i=x; i<x+w; i++) {
    for (j=y; j<y+h; j++) {
...
void ST7565_init(void) {
	...
	//  C86 = LOW 8080 ; C86 = HIGH 6800
	HAL_GPIO_WritePin(C86_GPIO_Port, C86_Pin, 0);
...
void ST7565_command(uint8_t c) {
	HAL_GPIO_WritePin(Display_GPIO_Port, A0_Pin, 0);
	HAL_GPIO_WritePin(Display_GPIO_Port, RD_Pin, 1);
	HAL_GPIO_WritePin(Display_GPIO_Port, WR_Pin, 0);

	//Send 8 bit command by MSB PORT
	uint16_t valor = c<<8;
	MODIFY_REG(Display_GPIO_Port->ODR, 0x0000FF00,valor);

	HAL_GPIO_WritePin(Display_GPIO_Port, CS_Pin, 0);
	HAL_GPIO_WritePin(Display_GPIO_Port, CS_Pin, 1);

}

void ST7565_data(uint8_t c) {
	HAL_GPIO_WritePin(Display_GPIO_Port, A0_Pin, 1);
	HAL_GPIO_WritePin(Display_GPIO_Port, RD_Pin, 1);
	HAL_GPIO_WritePin(Display_GPIO_Port, WR_Pin, 0);
